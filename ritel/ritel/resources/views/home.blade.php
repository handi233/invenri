...
<body data-page="dashboard" class="admin-layout">
    ...
    <div class="admin-wrapper" id="admin-wrapper">
        ...
        <main class="admin-main">
                            @if($stock->where('stock', '<', 10)->count() > 0)
                    <div class="alert alert-warning d-flex align-items-center" role="alert">
                        <i class="bi bi-exclamation-triangle-fill me-2 fs-4"></i>
                        <div>
                            <strong>Peringatan!</strong> Ada {{ $stock->where('stock', '<', 10)->count() }} barang dengan stok kurang dari 10. Segera lakukan pengisian.
                        </div>
                    </div>
                @endif
                @if(session('sukses'))
            ...
            <div class="container-fluid p-4 p-lg-5">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h1 class="h3 mb-0">Dashboard | Stock Barang</h1>
                                    ...
                                    <table class="table table-hover mb-0 w-100">
                                        ...
                                        <tbody id="tableData">
                                             ...
                                             @foreach($stock as $barang)
                                                <tr @if($barang->stock < 10) class="table-danger" @endif>
                                                    <td>{{ $barang->id_brng }}</td>
                                                     <td>{{ $barang->id_stock_brng }}</td>
                                                    <td>{{ $barang->nm_brng }}</td>
                                                    <td>{{ $barang->deskripsi }}</td>
                                                    <td>
                                                          @if($barang->stock < 5)
                                                        <span class="badge bg-danger">Sangat Kurang </span>
                                                    @elseif($barang->stock < 10)
                                                        <span class="badge bg-warning text-dark">Kurang ({{ $barang->stock }})</span>
                                                    @else
                                                        {{ $barang->stock }}
                                                    @endif
                                                    </td>  
  ...
  <form id="myForm" action="" method="POST">
    <div class="mb-3">
     @csrf
      ...
      <input type="text" class="form-control" id="nameInput" name="nm_brng" placeholder="Nama Barang" required>
    </div>
    <div class="mb-3">
    ...
    <div class="mb-3">
      ...
      <input type="number" class="form-control" id="stock" name="stock" placeholder="Isi Stock" required min="1"></textarea>
    </div>
